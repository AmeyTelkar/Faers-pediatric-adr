...
/* ── Layer 5: Copyright Watermark (full-screen repeating tile) ──── */
function Watermark() {
  // Generate a grid of watermark tiles covering the full viewport
  const tiles = [];
  for (let i = 0; i < 12; i++) {
    tiles.push(
      <p key={i} style={{
        fontSize: 14, color: '#fff', fontFamily: 'monospace',
        letterSpacing: 4, fontWeight: 700, whiteSpace: 'nowrap',
        padding: '80px 40px',
      }}>
        PULSETECH ANC-031 • EDUCATIONAL USE ONLY
      </p>
    );
  }
  return (
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="h-7 bg-surface-card/50 border-t border-surface-border flex items-center justify-between px-4">
            ...
            <p className="text-[9px] text-gray-700 tracking-wide select-none font-mono">
              Educational Use Only | Unauthorized reproduction is prohibited
            </p>
          </div>
        </div>
      </div>

...
Post-marketing surveillance of adverse drug reactions (ADRs) in pediatric endocrine pharmacotherapy --- encompassing recombinant growth hormone (rhGH), thyroid hormone replacement, glucocorticoid supplementation, gonadotrophin-releasing hormone (GnRH) agonists, and anti-thyroid agents --- remains almost entirely descriptive. Existing pharmacovigilance studies apply disproportionality methods that quantify drug-level associations rather than predicting the clinical severity of individual reported outcomes. No supervised machine-learning framework has been developed for multiclass outcome-severity classification in pediatric endocrine \FAERS reports. This study addresses that gap by developing, validating, and benchmarking tabular machine-learning classifiers for seven-class outcome-severity prediction in a large pediatric endocrine cohort.
 
\par\vspace{1.5pt}\noindent\textbf{Methods:}
FDA Adverse Event Reporting System (\FAERS) quarterly data files spanning 20 consecutive quarters (2021Q1--2025Q4) were processed through a validated 13-step cleaning pipeline, deduplicated per FDA guidance, and restricted to the full pediatric age band (0--17 years, ICH E11(R1)). A unified endocrine cohort ($N = 44{,}431$ unique case reports) was constructed across five pre-specified drug classes: Somatropin ($n = 31{,}948$), Levothyroxine ($n = 6{,}523$), Hydrocortisone ($n = 5{,}035$), Leuprolide ($n = 1{,}970$), and Methimazole ($n = 283$). A 48-column engineered feature set was reduced to 14 core clinical columns (13 modelling predictors plus 1 target outcome). The seven-class outcome label (\texttt{outc\_cod}: CA, DE, DS, HO, LT, OT, RI), missing in 73.56\% of records, was resolved via a two-stage clinical imputation scheme comprising adverse-event-specific mode imputation (Stage~1) and hierarchical clinical-indicator fallback (Stage~2). Random Forest and XGBoost were benchmarked under leakage-aware evaluation using a stratified 80/20 holdout split.
 
\par\vspace{1.5pt}\noindent\textbf{Results:}
On the leakage-aware 14-column model, XGBoost achieved the best overall performance (accuracy 87.28\%, Macro-F1 0.4903), marginally outperforming Random Forest (accuracy 86.99\%, Macro-F1 0.4889). On the 15-column sanity-check model (with target-leakage \texttt{severity\_score} included), XGBoost reached 97.47\% accuracy (Macro-F1 0.9704), confirming clinical encoding integrity. The cohort exhibited pronounced class imbalance: Other Serious (OT) comprised 80.97\% of post-imputation reports, Hospitalisation (HO) 13.18\%, Death (DE) 2.35\%, Life-Threatening (LT) 2.46\%, Congenital Anomaly (CA) 0.66\%, Disability (DS) 0.35\%, and Required Intervention (RI) 0.02\% (11 reports). Serious outcomes (HO, LT, DE, DS combined) comprised 18.34\% of the cohort. The adverse-event landscape was dominated by device-related reports from injectable growth hormone delivery systems, fundamentally distinguishing this cohort from metabolic pharmacotherapy cohorts where pharmacological reactions predominate.
 
\par\vspace{1.5pt}\noindent\textbf{Conclusions:}
This study demonstrates that a reproducible 13-step \FAERS preprocessing pipeline and two-stage clinical imputation scheme generalise from adolescent metabolic pharmacotherapy to the full pediatric age spectrum (0--17 years) and a pharmacologically distinct endocrine therapeutic domain with substantially higher outcome-label missingness (73.56\% vs 50.71\%). XGBoost achieved 87.28\% accuracy on the leakage-aware model in a 44{,}431-patient cohort spanning five endocrine drug classes. The framework should be interpreted as a retrospective proof of concept for pharmacovigilance report severity prioritisation. Temporal validation, external evaluation on non-US databases, and prospective assessment are required before operational deployment.
 
\par\vspace{1.5pt}\noindent\textbf{Keywords:}
Pharmacovigilance; Adverse Drug Reaction Reporting Systems; Growth Hormone Therapy; Pediatric Endocrinology; Machine Learning; XGBoost; FAERS; Outcome Severity Classification.
\end{tcolorbox}
\twocolumn
...
\hline
\rowcolor{rowA} Pediatric FAERS Descriptive & Misu \etal~\cite{misu2023}, Chen \etal~\cite{chen2020thyroid} & Age-stratified FAERS cohorts; thyroid drug safety & Clinically focused; age-band characterisation; thyroid-specific signal detection & No ML classification; descriptive disproportionality only; no outcome imputation & \xmark\ (Descriptive) \\
\hline
\rowcolor{jpbiLight}
...
\textit{J Clin Endocrinol Metab}. 2023;108(5):1017--1033.
...
\bibitem{misu2023}
Misu T, Konishi H, Noda A, \textit{et al}.
Pediatric pharmacovigilance using FAERS: age-stratified analysis.
\textit{Pharmacoepidemiol Drug Saf}. 2023;32(6):640--649.
doi:10.1002/pds.5590
 
\bibitem{chen2022}
Chen H, Li X, Chen Z, \textit{et al}.
Graph neural networks for drug safety.
\textit{Brief Bioinform}. 2022;23(1):bbab495.
...
\bibitem{tian2025}
Tian Y, Chen X, Li M, \textit{et al}.
XGBoost-based adverse drug event prediction from FAERS.
\textit{Drug Saf}. 2025;48(3):289--303.
doi:10.1007/s40264-024-01489-3
 
\bibitem{farnoush2024}
Farnoush A, Azimian S, Graham DJ, Staffa J.
Random forest and deep learning approaches for FAERS outcome prediction.
\textit{J Am Med Inform Assoc}. 2024;31(4):891--901.
doi:10.1093/jamia/ocae012
 
\bibitem{gao2025}
Gao Z, Zhang Y, Chen L, \textit{et al}.
PreciseADR: heterogeneous graph attention for patient-level adverse drug reaction prediction.
\textit{Brief Bioinform}. 2025;26(1):bbae098.
doi:10.1093/bib/bbae098
 
\bibitem{shi2026}
Shi X, Wang J, Li Q, \textit{et al}.
MSAT: multi-scale attention network for traditional medicine adverse event signal prioritization.
\textit{J Cheminform}. 2026;18(1):12.
doi:10.1186/s13321-026-00912-5
...
Stochholm K, Berglund A, Juul S, Gravholt CH.
Safety of GnRH agonists in children: registry-based analysis.
\textit{Eur J Endocrinol}. 2020;182(3):297--305.
doi:10.1530/EJE-19-0740
 
\bibitem{chen2020thyroid}
Chen Y, Liu J, Li R, \textit{et al}.
Pharmacovigilance analysis of thyroid medications using FAERS.
\textit{Front Pharmacol}. 2020;11:587603.
doi:10.3389/fphar.2020.587603

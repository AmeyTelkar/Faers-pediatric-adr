...
Pediatric endocrine pharmacotherapy --- encompassing recombinant growth hormone, thyroid hormone replacement, glucocorticoid supplementation, gonadotrophin-releasing hormone (GnRH) agonists, and anti-thyroid agents --- represents a clinically heterogeneous pharmacological domain in which post-marketing surveillance of adverse drug reactions (ADRs) remains almost entirely descriptive. No supervised machine-learning framework has been specifically developed for multiclass outcome-severity prediction in pediatric endocrine-related \FAERS reports. We developed and benchmarked tabular learners for seven-class outcome-severity classification in a large pediatric endocrine cohort.
 
\par\vspace{1.5pt}\noindent\textbf{Methods:}
\FAERS quarterly files spanning 20 consecutive quarters (2021Q1--2025Q4) were deduplicated per FDA guidance and restricted to the full pediatric age band (0--17 years). A unified endocrine cohort ($n = 44{,}431$) was constructed across five drug classes: Somatropin ($n = 31{,}948$), Levothyroxine ($n = 6{,}523$), Hydrocortisone ($n = 5{,}035$), Leuprolide ($n = 1{,}970$), and Methimazole ($n = 283$). A 48-column engineered feature set was reduced to 14 core clinical columns (13 modeling predictors). The seven-class outcome label (\texttt{outc\_cod}), missing in 73.56\%, was resolved via two-stage adverse-event-specific mode imputation and clinical-indicator fallback. Random Forest and XGBoost were benchmarked for multiclass severity classification.
 
\par\vspace{1.5pt}\noindent\textbf{Results:}
On the fully imputed dataset, XGBoost achieved the best overall performance (accuracy 87.28\%, Macro-F1 0.4903) on the leakage-aware 14-column model, marginally outperforming Random Forest (accuracy 86.99\%, Macro-F1 0.4889). On the sanity-check 15-column model (with target-leakage severity score included), XGBoost reached 97.47\% accuracy with a Macro-F1 of 0.9704, confirming clinical encoding integrity. The cohort exhibited pronounced class imbalance: Other Serious (OT) comprised 80.97\% of post-imputation reports, while Required Intervention (RI) contained only 11 reports (0.02\%). Serious outcomes (hospitalisation, life-threatening events, death, disability) comprised 18.34\% of the cohort. The five drug classes showed markedly different adverse-event profiles: device-related reports dominated the Somatropin sub-panel (reflecting injectable delivery systems), while thyroid-related and metabolic adverse events characterised the Levothyroxine and Methimazole sub-panels.
 
\par\vspace{1.5pt}\noindent\textbf{Conclusions:}
XGBoost demonstrated the strongest overall severity classification performance in the pediatric endocrine cohort, achieving 87.28\% accuracy with clinically balanced minority-class discrimination. The high missingness in outcome labels (73.56\%) emphasises the critical importance of the two-stage clinical imputation scheme for maintaining cohort integrity. The framework extends our prior adolescent metabolic pharmacotherapy work to the full pediatric age spectrum and a distinct endocrine therapeutic domain, demonstrating the generalisability of the 13-step preprocessing pipeline and imputation methodology across pharmacological contexts. Temporal validation, external evaluation and prospective assessment are required before operational use.
 
\par\vspace{1.5pt}\noindent\textbf{Keywords:}
Pharmacovigilance; Adverse Drug Reaction Reporting Systems; Growth Hormone Therapy; Pediatric Endocrinology; Machine Learning; FAERS.
\end{tcolorbox}
\twocolumn
...
The 13-step pipeline specification is identical to that described in our prior work~\cite{telkar2026obesity}, with only the age filter (Step~6) reconfigured from 12--17 years (adolescent) to 0--17 years (full pediatric). Table~\ref{tab:pipeline} provides the full per-step specification adapted for the endocrine cohort.
